...
def test_crowdsale_initialized(chain, moss_crowdsale_pre, moss_coin, coin_owner, pre_period, min_invest, cap_pre, rate, token_decimals, invest_decimals, presale_bonus_ether, presale_bonus_rate):
    w3 = chain.web3
...
def test_over_cap(chain, moss_crowdsale_pre, moss_coin, coin_owner, accounts, rate, cap_pre, token_decimals):
    w3 = chain.web3
    moss_crowdsale_pre.transact({'from':accounts[1], 'value': cap_pre // 20000 * (10 ** token_decimals)}).buyTokens(accounts[1])

    with pytest.raises(TransactionFailed):
        moss_crowdsale_pre.transact({'from':accounts[4], 'value': cap_pre // 20000 * (10 ** token_decimals)}).buyTokens(accounts[4])

def test_cap_waiting(chain, moss_crowdsale_pre, moss_coin, coin_owner, accounts, rate, cap_pre, token_decimals):
    w3 = chain.web3
    moss_crowdsale_pre.transact({'from':accounts[1], 'value': cap_pre // 20000 * (10 ** token_decimals)}).buyTokens(accounts[1])
